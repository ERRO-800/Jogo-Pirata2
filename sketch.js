...
function ShowBoats() {
  if (boats.length>0) { 
    ...
    for (var i = 0; i < boats.length; i++) {  
      if (boats[i]) { //verificar se existe algo lá dentro.Undefined é igual a falso 
        Matter.Body.setVelocity(boats[i].body,{x:-0.9,y:0});
        boats[i].display();   //mostra na tela  
      }
    }
...
function collisionWithBoat(index){
  ...
  for (var i = 0; i < boats.length; i++) {  
    ...
    if (boats[i]!=undefined&&balls[index]!=undefined) { //verificar se existe algo lá dentro.Undefined é igual a falso 
